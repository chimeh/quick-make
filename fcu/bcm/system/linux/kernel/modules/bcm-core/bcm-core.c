/*
 * $Id: bcm-core.c 1.49 Broadcom SDK $
...
extern int port_led_load(void);
extern void port_led_scan_init(void);
...
static int bcm_phyid_scan(int unit, int flag)
{
	uint16 value;
	int phy_id;

	if (flag == 0) {
		for (phy_id = 0; phy_id < 255; phy_id++) {
			soc_miim_read(unit, phy_id, 0x2, &value);
			printk("==>: phy id 0x%x, reg addr 0x2, value 0x%x\r\n", phy_id, value);
			soc_miim_read(unit, phy_id, 0x3, &value);
			printk("==>: phy id 0x%x, reg addr 0x3, value 0x%x\r\n\r\n", phy_id, value);
		}
	} else if (flag == 1) {
		for (phy_id = 0; phy_id < 255; phy_id++) {
			soc_miimc45_read(unit, phy_id, 1, 0x2, &value);
			printk("==>: phy id 0x%x, reg addr 0x2, value 0x%x", phy_id, value);
			soc_miimc45_read(unit, phy_id, 1, 0x3, &value);
			printk("==>: phy id 0x%x, reg addr 0x3, value 0x%x", phy_id, value);
		}
	}
	return 0;
}
