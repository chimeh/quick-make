...
*/
int
hsl_msg_encode_tlv (u_char **pnt, u_int32_t *size, u_int16_t type,
		    u_int16_t length)
{
  u_char *sp = *pnt;
  ...
  length += HSL_MSG_TLV_HEADER_SIZE;
...
*/
int
hsl_msg_encode_if (u_char **pnt, u_int32_t *size, struct hal_msg_if *msg)
{
  ...
  for (i = 0; i < CINDEX_SIZE; i++)
	...
	switch (i)
	  ...
	  case HAL_MSG_CINDEX_IF_FLAGS:
	    hsl_msg_encode_tlv (pnt, size, i, 4);
	  ...
	  case HAL_MSG_CINDEX_IF_HW:
	    hsl_msg_encode_tlv (pnt, size, i, HAL_HW_LENGTH + 2 + 2);
	  ...
	  case HAL_MSG_CINDEX_IF_BANDWIDTH:
	    hsl_msg_encode_tlv (pnt, size, i, 8);
        TLV_ENCODE_PUT(&msg->bandwidth, 8);
	    break;
      ...
      case HAL_MSG_CINDEX_IF_SEC_HW_ADDRESSES:
        hsl_msg_encode_tlv (pnt, size, i, 2 + msg->nAddrs * msg->hw_addr_len);
...
*/
int
hsl_msg_decode_if (u_char **pnt, u_int32_t *size, struct hal_msg_if *msg)
{
  struct hsl_msg_tlv_header tlv;
  unsigned char *sp = *pnt;
  u_int16_t i;
  ...
  while (*size)
	...
	case HAL_MSG_CINDEX_IF_HW:
 	...
 	case HAL_MSG_CINDEX_IF_ARP_AGEING_TIMEOUT:
 	  ...
 	  break;	  
	case HAL_MSG_CINDEX_IF_BANDWIDTH:
	  TLV_DECODE_GET (&msg->bandwidth, 8);
	  SET_CINDEX (msg->cindex, tlv.type);
	  break;
	case HAL_MSG_CINDEX_IF_AUTONEGO:
	  ...
	  break;
        case HAL_MSG_CINDEX_IF_SEC_HW_ADDRESSES:
          ...
          msg->addresses = oss_malloc (sizeof (u_int8_t *), OSS_MEM_HEAP);
          if (msg->addresses == NULL)
            goto CLEANUP;
          for (i = 0; i < msg->nAddrs; i++)
            {
              msg->addresses[i] = oss_malloc (msg->sec_hw_addr_len, OSS_MEM_HEAP);
...
int 
hsl_msg_decode_debug_hsl(u_char **pnt, u_int32_t *size, struct hal_msg_debug_hsl_req *msg)
{
   unsigned char *sp = *pnt;
   
    /* debug module name. */
   TLV_DECODE_GET (msg->module_str, HAL_DEBUG_HSL_NAME_LEN);
   
   /* module enable. */
   TLV_DECODE_GETW (msg->enable);
   
   /* module level */
   TLV_DECODE_GETW (msg->level);
...
*/
int
hsl_msg_encode_lacp_psc_set (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_psc_set *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_lacp_psc_set (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_psc_set *msg)
{
  unsigned char *sp = *pnt;

  memset(msg, 0, sizeof (struct hal_msg_lacp_psc_set));
...
int
hsl_msg_decode_lacp_global_psc_set (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_global_psc_set *msg)
{
  unsigned char *sp = *pnt;

  memset(msg, 0, sizeof (struct hal_msg_lacp_global_psc_set));

  /* Check size. */
  if (*size < HAL_MSG_LACP_GLOBAL_PSC_SIZE)
    return HAL_MSG_PKT_TOO_SMALL;
...
*/
int
hsl_msg_encode_lacp_id (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_agg_identifier *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_lacp_id (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_agg_identifier *msg)
{
  unsigned char *sp = *pnt;
  ...
  memset (msg, 0, sizeof (struct hal_msg_lacp_agg_identifier));
...
*/
int
hsl_msg_encode_lacp_agg_add (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_agg_add *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_lacp_agg_add (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_agg_add *msg)
{
  unsigned char *sp = *pnt;
  ...
  memset (msg, 0, sizeof (struct hal_msg_lacp_agg_add));
...
*/
int
hsl_msg_encode_lacp_mux (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_mux *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_lacp_mux (u_char **pnt, u_int32_t *size, struct hal_msg_lacp_mux *msg)
{
  unsigned char *sp = *pnt;
  ...
  memset (msg, 0, sizeof (struct hal_msg_lacp_mux));
...
*/
int
hsl_msg_encode_vlan_classifier_rule(u_char **pnt, u_int32_t *size, struct hal_msg_vlan_classifier_rule *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_vlan_classifier_rule(u_char **pnt, u_int32_t *size, struct hal_msg_vlan_classifier_rule *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_igs_entry(u_char **pnt, u_int32_t *size, struct hal_msg_igmp_snoop_entry *msg)
{
  unsigned char *sp = *pnt;
  u_int16_t index;
...
int
hsl_msg_decode_igs_entry(u_char **pnt, u_int32_t *size, struct hal_msg_igmp_snoop_entry *msg,
                         void *(*mem_alloc)(u_int32_t size))
{
  ...
  msg->ifindexes = (unsigned int *)mem_alloc(msg->count * sizeof(int));
  if(!msg->ifindexes)
...
*/
int
hsl_msg_encode_igs_bridge(u_char **pnt, u_int32_t *size, struct hal_msg_igs_bridge *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_igs_bridge(u_char **pnt, u_int32_t *size, struct hal_msg_igs_bridge *msg)
{
  unsigned char *sp = *pnt;
  ...
  memset(msg, 0, sizeof (struct hal_msg_igs_bridge));
...
*/
int
hsl_msg_encode_mlds_bridge(u_char **pnt, u_int32_t *size,
...
*/
int
hsl_msg_decode_mlds_bridge(u_char **pnt, u_int32_t *size,
                            struct hal_msg_mlds_bridge *msg)
{
  unsigned char *sp = *pnt;
  ...
  memset(msg, 0, sizeof (struct hal_msg_mlds_bridge));
...
int
hsl_msg_encode_mlds_entry(u_char **pnt, u_int32_t *size,
...
int
hsl_msg_decode_mlds_entry(u_char **pnt, u_int32_t *size,
                          struct hal_msg_mld_snoop_entry *msg,
                          void *(*mem_alloc)(u_int32_t size))
{
  unsigned char *sp = *pnt;
  ...
  msg->ifindexes = (int *)mem_alloc(msg->count * sizeof(int));
  if(!msg->ifindexes)
     return HAL_MSG_MEM_ALLOC_ERROR; 
...
*/
int
hsl_msg_encode_l2_fdb_resp (u_char **pnt, u_int32_t *size, struct hal_msg_l2_fdb_entry_resp *msg)
{
  unsigned char *sp = *pnt;
  u_int16_t index;
...
*/
int
hsl_msg_decode_l2_fdb_resp (u_char **pnt, u_int32_t *size, struct hal_msg_l2_fdb_entry_resp *msg)
{
  unsigned char *sp = *pnt;
  int index;
...
*/
int
hsl_msg_encode_l2_fdb_req (u_char **pnt, u_int32_t *size, struct hal_msg_l2_fdb_entry_req *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_l2_fdb_req (u_char **pnt, u_int32_t *size, struct hal_msg_l2_fdb_entry_req *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_arp_cache_resp (u_char **pnt, u_int32_t *size, struct hal_msg_arp_cache_resp *msg)
{
  unsigned char *sp = *pnt;
  u_int16_t i;
  ...
  for (i = 0; i < msg->count; i++)
    {
       ...
       /* age_time */
       TLV_ENCODE_PUTL (msg->cache[i].age_time);
     }
   return *pnt - sp;
}
...
*/
int
hsl_msg_decode_arp_cache_resp (u_char **pnt, u_int32_t *size, struct hal_msg_arp_cache_resp *msg)
{
  unsigned char *sp = *pnt;
  int i;
...
*/
int
hsl_msg_encode_arp_cache_req (u_char **pnt, u_int32_t *size, struct hal_msg_arp_cache_req *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_arp_cache_req (u_char **pnt, u_int32_t *size, struct hal_msg_arp_cache_req *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_if_fib_bind_unbind (u_char **pnt, u_int32_t *size, 
...
*/
int
hsl_msg_encode_ipv4_addr(u_char **pnt, u_int32_t *size, struct hal_msg_if_ipv4_addr *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_ipv4_addr(u_char **pnt, u_int32_t *size, struct hal_msg_if_ipv4_addr *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_ipv6_nbr_cache_resp (u_char **pnt, u_int32_t *size, struct hal_msg_ipv6_nbr_cache_resp *msg)
{
  unsigned char *sp = *pnt;
  u_int16_t i;
...
/* 
   Decode ipv6 neighbor cache resp. 
*/
int
hsl_msg_decode_ipv6_nbr_cache_resp (u_char **pnt, u_int32_t *size, 
...
/* 
   Encode ipv6 neighbor cache request.
*/
int
hsl_msg_encode_ipv6_nbr_cache_req (u_char **pnt, u_int32_t *size, struct hal_msg_ipv6_nbr_cache_req *msg)
{
  unsigned char *sp = *pnt;
...
/* 
   Decode ipv6 neighbor cache request. 
*/
int
hsl_msg_decode_ipv6_nbr_cache_req (u_char **pnt, u_int32_t *size, struct hal_msg_ipv6_nbr_cache_req *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_ipv6_addr(u_char **pnt, u_int32_t *size, struct hal_msg_if_ipv6_addr *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_ipv6_addr(u_char **pnt, u_int32_t *size, struct hal_msg_if_ipv6_addr *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_ipv4_route (u_char **pnt, u_int32_t *size, struct hal_msg_ipv4uc_route *msg)
{
  unsigned char *sp = *pnt;
  u_int16_t i;
  ...
  for (i = 0; i < msg->num; i++)
    {
      hsl_msg_encode_tlv (pnt, size, HAL_MSG_CINDEX_NEXTHOP, 13);
...
*/
int
hsl_msg_decode_ipv4_route (u_char **pnt, u_int32_t *size, struct hal_msg_ipv4uc_route *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_decode_ipv6_route (u_char **pnt, u_int32_t *size, struct hal_msg_ipv6uc_route *msg)
{
  unsigned char *sp = *pnt;
  struct hal_msg_tlv_header tlv;
  int num;
...
*/
int
hsl_msg_encode_ipv4_vif_add (u_char **pnt, u_int32_t *size, 
...
*/
int
hsl_msg_decode_ipv4_vif_add (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv4mc_vif_add *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_ipv4_mrt_add (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv4mc_mrt_add *msg)
{
  ...
  /* Olist TTLs */
  TLV_ENCODE_PUT (msg->olist_ttls, HAL_MAX_VIFS);
...
*/
int
hsl_msg_decode_ipv4_mrt_add (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv4mc_mrt_add *msg)
{
  unsigned char *sp = *pnt;
  ...
  /* Olist TTLs */
  TLV_DECODE_GET (msg->olist_ttls, HAL_MAX_VIFS);
...
*/
int
hsl_msg_encode_ipv4_mrt_del (u_char **pnt, u_int32_t *size, 
...
*/
int
hsl_msg_decode_ipv4_mrt_del (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv4mc_mrt_del *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_ipv4_sg_stat (u_char **pnt, u_int32_t *size, 
...
*/
int
hsl_msg_decode_ipv4_sg_stat (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv4mc_sg_stat *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_ipv6_vif_add (u_char **pnt, u_int32_t *size, 
...
*/
int
hsl_msg_decode_ipv6_vif_add (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv6mc_vif_add *msg)
{
  unsigned char *sp = *pnt;
...
*/
int
hsl_msg_encode_ipv6_mrt_add (u_char **pnt, u_int32_t *size, 
 ...
 */
int
hsl_msg_decode_ipv6_mrt_add (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv6mc_mrt_add *msg)
{
  unsigned char *sp = *pnt;
  int i = 0;
  ...
  /* Decode olist */
  msg->olist = oss_malloc ((msg->num_olist * sizeof (u_int16_t)), 
      OSS_MEM_HEAP);
  if (msg->olist == NULL)
    return -1;
...
*/
int
hsl_msg_encode_ipv6_mrt_del (u_char **pnt, u_int32_t *size, 
...
*/
int
hsl_msg_decode_ipv6_mrt_del (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv6mc_mrt_del *msg)
{
  unsigned char *sp = *pnt;
...
int
hsl_msg_encode_ipv6_sg_stat (u_char **pnt, u_int32_t *size, 
...
*/
int
hsl_msg_decode_ipv6_sg_stat (u_char **pnt, u_int32_t *size, 
    struct hal_msg_ipv6mc_sg_stat *msg)
{
  unsigned char *sp = *pnt;
