...
#if 0
void
hsl_sub_uint64(ut_int64_t *thea,ut_int64_t *theb, ut_int64_t *res)
{
  ut_int64_t tmpa = *thea;
  ut_int64_t tmpb = *theb;
  ut_int64_t *a;
  ut_int64_t *b;
  a = &tmpa;
  b = &tmpb;
  ...
  {                                       
    ...
    return hsl_add_uint64(res, b, res);
  }                                       
  if(a->l[0] < b->l[0])                  
  {                                  
...
#else
void hsl_sub_uint64(ut_int64_t *thea,ut_int64_t *theb, ut_int64_t *res)
{
    if(thea == NULL || theb == NULL || res == NULL) {
        return ;
    }

    if(thea->ll[0] < theb->ll[0]) {
        res->ll[0] = theb->ll[0] - thea->ll[0];
    } else {
        res->ll[0] = thea->ll[0] - theb->ll[0];
    }
...
hsl_free(void *ptr)
{
  return oss_free(ptr,OSS_MEM_HEAP); 
}
