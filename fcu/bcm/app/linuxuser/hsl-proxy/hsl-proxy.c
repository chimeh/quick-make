...
static void hsl_proxy_usage(char *progname, int status) {
...
static void hsl_proxy_exception_handler(int signo, siginfo_t *siginfo, void *context)
{ }
void signal_install() {
    ...
    act.sa_sigaction = hsl_proxy_exception_handler;
...
{\
    hsl_proxy_log_debug("version %s.%s.%s.%s, compiled by %s, %s %s \n\r", \
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        case 'v':
            ...
            break;
        case 'h':
            hsl_proxy_usage(progname, 0);
            break;
        default:
            hsl_proxy_usage(progname, 1);
            break;
        }
    }
    ...
    /* Create VTY socket */
    vty_serv_sock(vty_addr, vty_port, "/tmp/.hsl_proxy.sock");
...
//      vty_read_str("no enable password", CONFIG_NODE);
        sprintf(hostname, "hostname HSL-PROXY");
