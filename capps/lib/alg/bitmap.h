...
#ifndef _ZEBOS_BITMAP_H
#define _ZEBOS_BITMAP_H
...
#define BITMAP_LAST_BLOCK(bitmap,block ) \
  ( ( block->id == BITMAP_LAST_BLOCK_ID ( bitmap ) ) ? TRUE : FALSE)

#define BITMAP_FIRST_BLOCK(bitmap,block ) \
  ( ( block->id ==  BITMAP_FIRST_BLOCK_ID ( bitmap ) ) ? TRUE : FALSE)
