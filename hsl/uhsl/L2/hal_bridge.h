...
struct hal_bridge_ops
{
	int (*bridge_init)(u_int32_t slot_mask);
	int (*bridge_deinit)(u_int32_t slot_mask);
	int (*bridge_add )(char *name, unsigned int is_vlan_aware,
                enum hal_bridge_type type, unsigned char edge,
                unsigned char beb, unsigned char *mac, u_int32_t slot_mask);
	int (*bridge_delete) (char *name, u_int32_t slot_mask);

	int (*bridge_change_vlan_type) (char *name, int is_vlan_aware, u_int8_t type, u_int32_t slot_mask);
	int (*bridge_set_state) (char *name, u_int16_t enable, u_int32_t slot_mask);
	int (*bridge_set_ageing_time) (char *name, u_int32_t ageing_time, u_int32_t slot_mask);
	int (*bridge_disable_ageing) (char *name, u_int32_t slot_mask);
	int (*bridge_set_learning) (char *name, int learning, u_int32_t slot_mask);
	int (*bridge_add_port) (char *name, unsigned int ifindex, u_int32_t slot_mask);
	int (*bridge_delete_port) (char *name, int ifindex, u_int32_t slot_mask);
	int (*bridge_set_port_state) (char *bridge_name,
                           int ifindex, int instance, int state, u_int32_t slot_mask);
	int (*bridge_add_instance) (char *name, int instance, u_int32_t slot_mask);
	int (*bridge_delete_instance) (char *name, int instance, u_int32_t slot_mask);
	int (*bridge_add_vlan_to_instance) (char *name, int instance,
                                 unsigned short vid, u_int32_t slot_mask);
	int (*bridge_delete_vlan_from_instance) (char *name, int instance,
                                      unsigned short vid, u_int32_t slot_mask);
	int (*bridge_set_learn_fwd) (const char *const bridge_name, const int ifindex,
                          const int instance, const int learn,
                          const int forward, u_int32_t slot_mask);
	int (*bridge_set_proto_process_port) (const char *const bridge_name,
                                   const int ifindex,
                                   enum hal_l2_proto proto,
                                   enum hal_l2_proto_process process,
                                   u_int16_t vid, u_int32_t slot_mask);
	
	int (*pbb_dispatch_service_cnp) (char *br_name, unsigned int ifindex,
	                              unsigned isid, unsigned short svid_h,
	                              unsigned short svid_l, unsigned char srv_type, u_int32_t slot_mask);

	int (*pbb_dispatch_service_vip) (char *br_name, unsigned int ifindex,
	                              unsigned isid, unsigned char *macaddr,
	                              unsigned char srv_type, u_int32_t slot_mask);

	int (*pbb_dispatch_service_pip) (char *br_name, unsigned int ifindex,
	                              unsigned isid, u_int32_t slot_mask);


	int (*pbb_dispatch_service_cbp) (char *br_name, unsigned int ifindex,
	...
	int (*pbb_remove_service) (char *br_name, unsigned int ifindex, unsigned isid, u_int32_t slot_mask);


	int (*l2_qos_set_cos_preserve) (const int ifindex,
	                             u_int16_t vid, u_int8_t preserve_ce_cos, u_int32_t slot_mask);



	int (*l2_get_index_by_mac_vid_svid) (char *bridge_name, int *ifindex, char *mac,
	                                  u_int16_t vid, u_int16_t svid, u_int32_t slot_mask);	
};
...
*/
int
hal_bridge_add (char *name, unsigned int is_vlan_aware,
                enum hal_bridge_type type, unsigned char edge,
...
int
hal_pbb_dispatch_service_cnp (char *br_name, unsigned int ifindex,
                              unsigned isid, unsigned short svid_h,
                              unsigned short svid_l, unsigned char srv_type);


int
hal_pbb_dispatch_service_vip (char *br_name, unsigned int ifindex,
                              unsigned isid, unsigned char *macaddr,
