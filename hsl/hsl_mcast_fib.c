...
*/
void
hsl_mc_fib_tree_release(int  family)
{
                        ...
                        u_int16_t count, u_int32_t *ifindexes, char is_exclude)
{
  ...
  u_char mac_addr[HSL_ETHER_ALEN];               /* Mac address corresponds to a group. */
  ...
  hsl_convert_ip_to_hw (&HSL_MCAST_FIB_GROUP4, mac_addr);
  ...
  for(index = 0; index < count; index++) 
  {
    ...
    ret =  hsl_bridge_add_fdb(bridge_name, ifindexes[index], (char *)mac_addr, 
                        ...
                        char is_exclude)
{
  ...
  for(index = 0; index < count; index++) 
  {
    ...
    hsl_bridge_delete_fdb(bridge_name, ifindexes[index], (char *)mac_addr, HSL_ETHER_ALEN, vid, 0);
  }
 ...
 if((!HSL_PREFIX_IS_ZERO(&HSL_MCAST_FIB_SRC))) 
  ...
  for(index = 0; index < count; index++) 
  {
    ...
    _hsl_mc_igs_add_del_if( (! HSL_IPV6_ZERO_ADDR (*src) && top_node) ?
                            ...
                            &grp_src_entry, vid, ifindexes[index],
                            HSL_IPV6_ZERO_ADDR (*src) ? HSL_MCAST_FIB_ACTUAL:
                            HSL_MCAST_FIB_INHERITED, HSL_MCAST_FIB_DEL, HSL_FALSE);
                    ...
                    u_int32_t iif_vif,u_int32_t num_olist,u_char *olist_ttls)
{
  ...
  u_int32_t registered_vif = 0;      /* Registered VIF.                     */   
  ...
  if(pim_running)
  {
    ...
    /* If iif is register vif, just return OK,  
       RP case - trap everything to CPU anyway. */
                      ...
                      u_int32_t iif_vif,u_int32_t num_olist,u_char *olist_ttls)
{
  ...
  if (p_hsl_ipv4_mc_db->hw_cb && p_hsl_ipv4_mc_db->hw_cb->hw_ipv4_mc_route_add)
	...
	{
#ifdef  HAVE_MCAST_IPV4 
	  if (p_hsl_ipv4_mc_db->os_cb && p_hsl_ipv4_mc_db->os_cb->os_ipv4_mc_route_del)
                      ...
                      u_int16_t *olist)
{
...
#ifdef  HAVE_MCAST_IPV6  
  /* Add it to the os. */
  if (p_hsl_ipv6_mc_db->os_cb && p_hsl_ipv6_mc_db->os_cb->os_ipv6_mc_route_add)
    {
  ...
  if (p_hsl_ipv6_mc_db->hw_cb && p_hsl_ipv6_mc_db->hw_cb->hw_ipv6_mc_route_add)
	...
	{
#ifdef  HAVE_MCAST_IPV6  
	  if (p_hsl_ipv6_mc_db->os_cb && p_hsl_ipv6_mc_db->os_cb->os_ipv6_mc_route_del)
