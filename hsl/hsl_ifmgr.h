...
#define HSL_L2_IFINDEX_TOTAL                        300
...
#define HSL_IF_BW_UNIT_MEGA                         1000000ULL
...
typedef struct 
{
  ...
  long long unsigned int         speed;          /* Speed. */
  u_int32_t                      mtu;            /* MTU. */
  u_int32_t                      duplex;         /* DUPLEX. */
  u_int32_t                      autonego;       /* AUTONEGO. */
  long long unsigned int         bandwidth;      /* BANDWIDTH. */
...
struct hsl_if
{
  ...
  bool is_agg_member;              /*is agg_member  modified by chentao add*/
...
#define HSL_IFMGR_IF_L3_DELETE       (1 << 1)
  ...
  u_int32_t if_property;             /* Type. */
    ...
    /* added to store ADS ngn settings */
    u_int32_t ngn_type_enabled; /* 1: enable ngn_type, 0: disable ngn_type */
    u_int32_t ngn_type;         /* ngn type, FPGA default value: 0x8871 */
...
int hsl_ifmgr_L2_ethernet_register (char *name, hsl_mac_address_t mac, u_int16_t mtu, u_int32_t speed, u_int32_t duplex, u_int32_t flags,	void *sys_info,	struct hsl_if **ppifp);
...
int hsl_ifmgr_set_cust_callbacks (struct hsl_ifmgr_cust_callbacks *cb);
int hsl_ifmgr_unset_cust_callbacks (void);
...
void hsl_ifmgr_sub_mac_counters(struct hal_if_counters *a,
                                struct hal_if_counters *b,
                                struct hal_if_counters *res);
int hsl_ifmgr_L2_ethernet_create (char *name, hsl_mac_address_t mac, u_int16_t mtu, u_int32_t speed, u_int32_t duplex, u_int32_t flags, void *sys_info, int send_notification, struct hsl_if **ppifp);
...
int hsl_if_cust_cb_register(void);
int hsl_if_cust_cb_unregister(void);
...
int hsl_ifmgr_set_bandwidth (hsl_ifIndex_t ifindex, long long unsigned int bandwidth, HSL_BOOL send_notification);
...
int hsl_ifmgr_lacp_global_psc_set (int psc);
