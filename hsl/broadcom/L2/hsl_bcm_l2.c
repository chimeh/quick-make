...
hsl_bcm_set_learning (struct hsl_bridge *b, int learn)
{
  ...
  flags = (learn) ? BCM_PORT_LEARN_ARL|BCM_PORT_LEARN_FWD : BCM_PORT_LEARN_FWD;
...
int
hsl_bcm_set_if_mac_learning (struct hsl_if *ifp, int disable)
{
  struct hsl_bcm_if *bcmifp;
  int flags, ret;

  HSL_FN_ENTER ();
  flags = (disable) ? BCM_PORT_LEARN_FWD : BCM_PORT_LEARN_ARL|BCM_PORT_LEARN_FWD;
  if (ifp->type != HSL_IF_TYPE_L2_ETHERNET)
    {
  ...
  if (hsl_bcm_bridge_p->stg[instance] != -1)
    {
	 ...
	 if (ret < 0) {
	 	HSL_LOG (HSL_LOG_PLATFORM, HSL_LEVEL_ERROR, "stp set state error! gport=%d, instance=%d, state=%d\n", gport, instance, ctc_port_state);
	 	HSL_FN_EXIT (-1);
	 }
...
hsl_bcm_set_stp_port_state (struct hsl_bridge *b, struct hsl_bridge_port *port, int instance, int state)
{
  ...
  if (ifp->type == HSL_IF_TYPE_L2_ETHERNET && ctcifp->trunk_id >= 0)
    {
       ...
       /* If L2 port is directly mapped to a L3(router port) set the state to
        ...
        if (ifp->type == HSL_IF_TYPE_L2_ETHERNET 
            && ! CHECK_FLAG (ifp->if_flags, HSL_IFMGR_IF_DIRECTLY_MAPPED))
          {
            ...
            ret = ctc_stp_set_state (gport, instance, CTC_STP_BLOCKING);
			if ( ret < 0) {
				HSL_FN_EXIT (-1);
				HSL_IFMGR_UNLOCK;
			}
...
hsl_bcm_add_vlan_to_instance (struct hsl_bridge *b, int instance, hsl_vid_t vid)
{
  ...
  if (ret < 0)
    {
      HSL_LOG (HSL_LOG_PLATFORM, HSL_LEVEL_ERROR, "Error adding VLAN %d \n", vid);
      HSL_FN_EXIT (-1);
    }
...
hsl_bcm_delete_vlan_from_instance (struct hsl_bridge *b, int instance, hsl_vid_t vid)
{
  ...
  //此处删除VLAN执行的是覆盖操作，删除instance中的vid只需要将vid加入到默认实例0中
  ret = ctc_stp_get_vlan_stpid(vid, 0)
  if (ret < 0)
    {
			    ...
			    u_int16_t enable_ingress_filter)
{
  ...
  port->type = port_type;
	...
	HSL_LOG (HSL_LOG_PLATFORM, HSL_LEVEL_DEBUG, "bcmx_port_discard_set mode = %d, ret = %d\n", mode, ret);
				...
				int *rxpause, int *txpause)
{
  ...
  bcmx_lport_t lport;
  int rx, tx;
  long long unsigned int rxv, txv;
...
hsl_bcm_flush_fdb (struct hsl_bridge *b, struct hsl_if *ifp, hsl_vid_t vid)
{
  ...
  if ((!vid) && (ifp)){
  	if(lport != BCMX_LPORT_INVALID)
		...
		ret = bcmx_l2_addr_delete_by_trunk (bcmifp->trunk_id, 0);
	HSL_LOG (HSL_LOG_FDB, HSL_LEVEL_INFO, "ret1 = %d\r\n", ret);
  	}
  /* Delete by logical port & vlan id. */
  	...
  	if(lport != BCMX_LPORT_INVALID)
		...
		ret = bcmx_l2_addr_delete_by_vlan_trunk(vid, bcmifp->trunk_id, 0);
	HSL_LOG (HSL_LOG_FDB, HSL_LEVEL_INFO, "ret2 = %d\r\n", ret);
  	}
  else 
    HSL_FN_EXIT (-1);  
...
hsl_bridge_hw_register (void)
{
  ...
  hsl_bcm_l2_cb.set_if_mac_learning = hsl_bcm_set_if_mac_learning;
