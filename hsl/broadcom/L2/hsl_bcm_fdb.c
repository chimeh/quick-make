...
hsl_bcm_fdb_addr_register (bcmx_l2_addr_t *addr, int insert, void *cookie)
{
  ...
  if (BCMX_LPORT_VALID (addr->lport))
    {
      /*cpu口，用3层接口映射*/
      if (BCM_GPORT_IS_MODPORT(addr->lport)) {
        HSL_LOG (HSL_LOG_FDB, HSL_LEVEL_INFO, "a mac added on cpu port. mac:%02x.%02x.%02x.%02x.%02x.%02x, vid:%d\n",
                                        addr->mac[0],addr->mac[1],addr->mac[2],addr->mac[3],addr->mac[4],addr->mac[5],
                                        addr->vid);
        if (_SHR_GPORT_MODPORT_PORT_GET(addr->lport) == CMIC_PORT(addr->bcm_unit)) {
            BCM_GPORT_MODPORT_SET(addr->lport, addr->vid, 255);
        }
      }
  ...
  if (addr->flags & (BCM_L2_DISCARD_DST | BCM_L2_DISCARD_SRC)) {
 	entry.is_fwd = 0;
  } else {
  	/* Is forwarding? */
  	entry.is_fwd = 1;
  ...
  if (insert)
      ...
      if (ret != 0)
      ...
      else
        {
          ...
          if ((ret = hsl_get_fdb_entry (&eptr, SEARCH_BY_MAC, &key)) == 0)
            HSL_LOG (HSL_LOG_FDB, HSL_LEVEL_INFO,"Added [MAC:%x:%x:%x:%x:%x:%x] vid=%d to FDB\n",
  ...
  else
    {
      ...
      if (ret != 0)
        ...
        HSL_LOG (HSL_LOG_FDB, HSL_LEVEL_INFO,
                 "Deleted [MAC:%x:%x:%x:%x:%x:%x] vid=%d from FDB\n",
                 entry.mac_addr[0], entry.mac_addr[1], entry.mac_addr[2],
...
hsl_fdb_hw_cb_register (void)
{
	...
	ret = bcmx_l2_notify_register (hsl_bcm_fdb_addr_register, NULL);
  if (!ret)
