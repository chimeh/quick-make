...
extern struct list_head *qos_build_class_map_internal(cap_sub_info_t *sub, bcmx_lport_t lport, 
	void *ptr, 	int type, bool dir_in, int acl_entry_num);
extern struct list_head *qos_cap_alloc_cluster(cap_sub_info_t *sub, int num, bool dir_in, int chip_port);
extern struct list_head *qos_build_one_match_entry(cap_sub_info_t *sub,bool dir_in, bcmx_lport_t lport, qualify_func func, u32 qualify_value, u8 qualify_mask);
extern struct list_head *qos_build_acl(struct entry_msg_s *acl, cap_sub_info_t *sub,	bool dir_in, bcmx_lport_t lport, int acl_entry_num);
extern struct list_head *qos_build_class_map(u32 ifindex, void *ptr, int type, int sub_type, bool dir_in, int acl_entry_num);
extern struct list_head *qos_build_match_all_list(struct cm_entry *cm_entry, cap_info_t *cap, cap_sub_info_t *sub);

extern int qualify_in_port(bcm_field_entry_t eid, bcmx_lport_t lport, u8 no_use);
extern int qualify_in_vlan(bcm_field_entry_t eid, u16 vid, u8 no_use);
