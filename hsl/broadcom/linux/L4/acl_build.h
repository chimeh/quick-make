...
extern int bcm_build_acl_std(	bcm_field_entry_t eid, 
			struct std_access_entry *entry, cap_sub_info_t *sub);
extern int bcm_build_acl_ext_ip(bcm_field_entry_t eid,
	struct ext_ip_access_entry *entry, cap_sub_info_t *sub);
extern int bcm_build_acl_udp(bcm_field_entry_t eid, 
	struct ext_udp_access_entry *entry, cap_sub_info_t *sub);
extern int bcm_build_acl_tcp(bcm_field_entry_t eid, 
	struct ext_tcp_access_entry *entry, cap_sub_info_t *sub);
extern int bcm_build_acl_icmp(bcm_field_entry_t eid, 
	struct ext_icmp_access_entry *entry, cap_sub_info_t *sub);
extern int acl_build_action(cap_info_t *cap, bcm_field_entry_t eid, bool permit);
extern int bcm_default_acl_build(void);
