...
#else   /* EWAN linux2.6 */
static HLIST_HEAD(_mld_snoop_socklist);
#endif
static rwlock_t _mld_snoop_socklist_lock = RW_LOCK_UNLOCKED;

int mld_snoop_sock_create (struct socket *sock, int protocol);
/* Private packet socket structures. */

/* Forward declarations. */
static int _mld_snoop_sock_release (struct socket *sock);
static int _mld_snoop_sock_create (struct net *net, struct socket *sock, int protocol);
...
#else
static int _mld_snoop_sock_sendmsg (struct kiocb *iocb, struct socket *sock, struct msghdr *msg, int len);
static int _mld_snoop_sock_recvmsg (struct kiocb *iocb, struct socket *sock, struct msghdr *msg, int len, int flags);
#endif
...
_mld_snoop_sock_destruct (struct sock *sk)
{
  ...
  write_lock_bh (&_mld_snoop_socklist_lock);
#if 0   /* EWAN linux2.4 */
  for (skp = &_mld_snoop_socklist; *skp; skp = &(*skp)->next)
    {
      if (*skp == sk)
        {
          *skp = sk->next;
...
#else   /* EWAN linux2.6 */
    sk_for_each(s, node, &_mld_snoop_socklist) {
        if (s == sk) {
            sk_del_node_init(s);
            break;
        }
...
#endif
  write_unlock_bh (&_mld_snoop_socklist_lock);
...
_mld_snoop_sock_create (struct net *net, struct socket *sock, int protocol)
{
...
#if 0   /* EWAN linux2.4 */
  sk = sk_alloc (AF_MLD_SNOOP, GFP_KERNEL, 1);
#else   /* EWAN linux2.6 */
  sk = sk_alloc (net, AF_MLD_SNOOP, GFP_KERNEL, &mld_proto);
#endif
  if (sk == NULL)
    {
      return -ENOBUFS;
...
#if 0   /* EWAN linux2.4 */
  sk->family = AF_MLD_SNOOP;
  sk->num = protocol;   /* NETFORD :: Maybe sk->num Not be used in 2.6 ..
                           reference vcc_create() */
  sk->destruct = _mld_snoop_sock_destruct;
#else   /* EWAN linux2.6 */
  sk->sk_family = AF_MLD_SNOOP;
  sk->sk_protocol = protocol;   /* NETFORD :: Not Certainly.. */
  sk->sk_destruct = _mld_snoop_sock_destruct;
#endif
  ...
  write_lock_bh (&_mld_snoop_socklist_lock);
#if 0   /* EWAN linux2.4 */
  sk->next = _mld_snoop_socklist;
  _mld_snoop_socklist = sk;
#else   /* EWAN linux2.6 */
  sk_add_node(sk, &_mld_snoop_socklist);
#endif
  write_unlock_bh (&_mld_snoop_socklist_lock);
  return 0;
}
...
#else
_mld_snoop_sock_sendmsg (struct kiocb *iocb, struct socket *sock, struct msghdr *msg, int len)
#endif
{
  bcm_pkt_t *pkt = NULL;
  int ret = 0;
...
#else
_mld_snoop_sock_recvmsg (struct kiocb *iocb, struct socket *sock, struct msghdr *msg, int len, int flags)
#endif
{
...
#else
  ret1 = hsl_put_cmsg (msg, skb->data + size, skml.port);
#endif
...
#if 0   /* EWAN linux2.4 */
  for (sk = _mld_snoop_socklist; sk; sk = sk->next)
#else   /* EWAN linux2.6 */
  sk_for_each(sk, node, &_mld_snoop_socklist)
#endif
    {
      /* Skip dead sockets. */
#if 0   /* EWAN linux2.4 */
...
hsl_af_mlds_sock_init (void)
{
  ...
  int ret;
  ret = sock_register (&mld_snoop_family_ops);
  printk("mld sk = %d\r\n", ret);
  return 0;
}
