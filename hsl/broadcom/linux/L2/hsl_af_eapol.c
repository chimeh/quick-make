...
#else   /* EWAN linux2.6 */
static HLIST_HEAD(_eapol_socklist);
#endif
static rwlock_t _eapol_socklist_lock = __RW_LOCK_UNLOCKED(_eapol_socklist_lock);//RW_LOCK_UNLOCKED;

int eapol_sock_create (struct socket *sock, int protocol);
/* Private packet socket structures. */

/* Forward declarations. */
static int _eapol_sock_release (struct socket *sock);
static int _eapol_sock_create (struct net *net, struct socket *sock, int protocol);
...
_eapol_sock_destruct (struct sock *sk)
{
  ...
  write_lock_bh (&_eapol_socklist_lock);
#if 0   /* EWAN linux2.4 */
  for (skp = &_eapol_socklist; *skp; skp = &(*skp)->next)
    {
      if (*skp == sk)
        {
          *skp = sk->next;
...
#else   /* EWAN linux2.6 */
    sk_for_each(s, node, &_eapol_socklist) {
        if (s == sk) {
            sk_del_node_init(s);
            break;
        }
...
#endif
  write_unlock_bh (&_eapol_socklist_lock);
...
static struct proto eapol_proto = {
        .name     = "EAP",
        .owner    = THIS_MODULE,
...
_eapol_sock_create (struct net *net, struct socket *sock, int protocol)
{
...
#if 0   /* EWAN linux2.4 */
  sk = sk_alloc (AF_EAPOL, GFP_KERNEL, 1);
#else   /* EWAN linux2.6 */
  sk = sk_alloc (net, AF_EAPOL, GFP_KERNEL, &eapol_proto);
#endif
  if (sk == NULL)
    {
      return -ENOBUFS;
  ...
  sk->family = AF_EAPOL;
  sk->num = protocol;
  sk->destruct = _eapol_sock_destruct;
#else   /* EWAN linux2.6 */
  sk->sk_family = AF_EAPOL;
  sk->sk_protocol = protocol;
  sk->sk_destruct = _eapol_sock_destruct;
#endif
  ...
  write_lock_bh (&_eapol_socklist_lock);
#if 0   /* EWAN linux2.4 */
  sk->next = _eapol_socklist;
  _eapol_socklist = sk;
#else   /* EWAN linux2.6 */
  sk_add_node(sk, &_eapol_socklist);
#endif
  write_unlock_bh (&_eapol_socklist_lock);
  return 0;
}
...
#else
_eapol_sock_sendmsg (struct kiocb *iocb ,struct socket *sock, struct msghdr *msg, size_t len) 
#endif
{
  bcm_pkt_t *pkt = NULL;
  int ret = 0;
...
#else
_eapol_sock_recvmsg (struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len, int flags)
#endif
{
...
#if 0   /* EWAN linux2.4 */
  for (sk = _eapol_socklist; sk; sk = sk->next)
#else   /* EWAN linux2.6 */
  sk_for_each(sk, node, &_eapol_socklist)
#endif
    {
      /* Skip dead sockets. */
#if 0   /* EWAN linux2.4 */
...
{
	int ret;
  ret = sock_register (&eapol_family_ops);
  printk("eapol sk = %d\r\n", ret);
  return 0;
}
