...
#else   /* EWAN linux2.6 */
static HLIST_HEAD(_igmp_snoop_socklist);
#endif
static rwlock_t _igmp_snoop_socklist_lock = __RW_LOCK_UNLOCKED(_igmp_snoop_socklist_lock);//RW_LOCK_UNLOCKED;

int igmp_snoop_sock_create (struct socket *sock, int protocol);
/* Private packet socket structures. */

/* Forward declarations. */
static int _igmp_snoop_sock_release (struct socket *sock);
static int _igmp_snoop_sock_create (struct net *net, struct socket *sock, int protocol);
#if 0   /* EWAN linux2.4 */
...
_igmp_snoop_sock_destruct (struct sock *sk)
{
  ...
  write_lock_bh (&_igmp_snoop_socklist_lock);
#if 0   /* EWAN linux2.4 */
  for (skp = &_igmp_snoop_socklist; *skp; skp = &(*skp)->next)
    {
      if (*skp == sk)
        {
          *skp = sk->next;
...
#else   /* EWAN linux2.6 */
    sk_for_each(s, node, &_igmp_snoop_socklist) {
        if (s == sk) {
            sk_del_node_init(s);
            break;
        }
...
#endif
  write_unlock_bh (&_igmp_snoop_socklist_lock);
...
static struct proto igmp_proto = {
        .name     = "IGMP",
        .owner    = THIS_MODULE,
...
_igmp_snoop_sock_create (struct net *net, struct socket *sock, int protocol)
{
...
#if 0   /* EWAN linux2.4 */
  sk = sk_alloc (AF_IGMP_SNOOP, GFP_KERNEL, 1);
#else   /* EWAN linux2.6 */
  sk = sk_alloc (net, AF_IGMP_SNOOP, GFP_KERNEL, &igmp_proto);
#endif
  if (sk == NULL)
    {
      return -ENOBUFS;
...
#if 0   /* EWAN linux2.4 */
  sk->family = AF_IGMP_SNOOP;
  sk->num = protocol;
  sk->destruct = _igmp_snoop_sock_destruct;
#else   /* EWAN linux2.6 */
  sk->sk_family = AF_IGMP_SNOOP;
  sk->sk_protocol = protocol;
  sk->sk_destruct = _igmp_snoop_sock_destruct;
#endif
  ...
  write_lock_bh (&_igmp_snoop_socklist_lock);
#if 0   /* EWAN linux2.4 */
  sk->next = _igmp_snoop_socklist;
  _igmp_snoop_socklist = sk;
#else   /* EWAN linux2.6 */
  sk_add_node(sk, &_igmp_snoop_socklist);
#endif
  write_unlock_bh (&_igmp_snoop_socklist_lock);
  return 0;
}
...
#else
_igmp_snoop_sock_sendmsg (struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len)
#endif
{
  bcm_pkt_t *pkt = NULL;
  int ret = 0;
...
#else
_igmp_snoop_sock_recvmsg (struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len, int flags)
#endif
{
...
#if 0   /* EWAN linux2.4 */
  for (sk = _igmp_snoop_socklist; sk; sk = sk->next)
#else   /* EWAN linux2.6 */
  sk_for_each(sk, node, &_igmp_snoop_socklist)
#endif
    {
      /* Skip dead sockets. */
#if 0   /* EWAN linux2.4 */
...
hsl_af_igs_sock_init (void)
{
  ...
  int ret;
  ret = sock_register (&igmp_snoop_family_ops);
  printk("igmp sk = %d\r\n", ret);
  return 0;
}
