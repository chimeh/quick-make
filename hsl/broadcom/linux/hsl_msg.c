...
_hsl_map_if (struct hsl_if *ifp, struct hal_msg_if *msg, u_int32_t cindex)
{
  ...
  switch (ifp->type)
	...
	if (CHECK_CINDEX (cindex, HAL_MSG_CINDEX_IF_BANDWIDTH))
	  {	
	    msg->bandwidth = (ifp->u.l2_ethernet.speed / 8) * 1000;
...
hsl_msg_recv_l2_fdb_count_get (struct socket *sock, struct hal_nlmsghdr *hdr, char *msgbuf)
{
  ...
  /* Post the message. */
  ret = hsl_sock_post_msg (sock, HAL_MSG_L2_FDB_COUNT_GET, 0, hdr->nlmsg_seq, &resp, sizeof(resp));
...
int
hsl_msg_recv_lacp_nuc_psc_set (struct socket *sock, struct hal_nlmsghdr *hdr, 
			   char *msgbuf)
{
  u_char *pnt;
  u_int32_t size;
  struct hal_msg_lacp_nuc_psc_set msg;
  int ret;
  ...
  ret = hsl_msg_decode_lacp_nuc_psc_set(&pnt, &size, &msg);

  if (ret < 0)
    goto CLEANUP;

  ret = hsl_ifmgr_lacp_nuc_psc_set (msg.psc);
                            ...
                            char *msgbuf)
{
  ...
  {
	 hsl_sock_post_msg(sock, HAL_MSG_GET_OPTICAL_MODULE_INFO, hdr->nlmsg_seq, 0, &optical_module_info, sizeof(struct hal_optical_module_info));
  }
  return 0;
}
