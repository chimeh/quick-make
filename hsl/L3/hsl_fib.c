/* Copyright (C) 2002-2005 IP Infusion, Inc. All Rights Reserved. */
...
#undef HSL_LOG_GENERAL
#define HSL_LOG_GENERAL HSL_LOG_FIB
...
int hsl_fib_direct_prefix_add(void *data)
{
    int ret = 0;
    HSL_FN_ENTER();
    ...
    if (HSL_FIB_HWCB_CHECK(hw_direct_prefix_add))
      ret = HSL_FIB_HWCB_CALL(hw_direct_prefix_add) (data);
    
    HSL_FN_EXIT(ret);

}
int hsl_fib_direct_prefix_del(void *data)
{
    int ret = 0;
    HSL_FN_ENTER();
    ...
    /* TODO check for limits in prefix table. */
    if (HSL_FIB_HWCB_CHECK(hw_direct_prefix_del))
      ret = HSL_FIB_HWCB_CALL(hw_direct_prefix_del) (data);
    
    HSL_FN_EXIT(ret);

}

int hsl_fib_direct_nh_add(void *data)
{
    int ret = 0;
    HSL_FN_ENTER();
    ...
    /* TODO check for limits in prefix table. */
    if (HSL_FIB_HWCB_CHECK(hw_direct_nh_add))
      ret = HSL_FIB_HWCB_CALL(hw_direct_nh_add) (data);
    
    HSL_FN_EXIT(ret); 
}

int hsl_fib_direct_nh_del(void *data)
{
    int ret = 0;
    HSL_FN_ENTER();
    ...
    /* TODO check for limits in prefix table. */
    if (HSL_FIB_HWCB_CHECK(hw_direct_nh_del))
      ret = HSL_FIB_HWCB_CALL(hw_direct_nh_del) (data);
...
hsl_fib_init (void)
{
  ...
  SYSTEM_INIT_CHECK(hsl_fib_hw_cb_register(), "fib hw cb register");

  /* Initialize FIb manager OS(TCP/IP) stack callbacks. */
  SYSTEM_INIT_CHECK(hsl_fib_os_cb_register (), "fib os cb register");
...
_hsl_fib_prune_exception_children (hsl_fib_id_t fib_id, struct hsl_route_node *rnp)
{
  ...
  if (rnp->l_left)
	...
	{
	  if (HSL_PREFIX_ENTRY_IS_EXCEPTION (rnt) && (!(HSL_PREFIX_ENTRY_IS_CPU (rnt))))
	    {
...
hsl_fib_add_to_hw (hsl_fib_id_t fib_id, struct hsl_route_node *rnp, struct hsl_nh_entry *nh)
{
  ...
  if (CHECK_FLAG (nh->flags, HSL_NH_ENTRY_VALID))
	...
	HSL_FN_EXIT(0);
	  ...
	  if(nh->ifp->if_property != HSL_IF_CPU_ONLY_INTERFACE) {
...
static int 
_hsl_fib_nh_delete2 (hsl_fib_id_t fib_id, struct hsl_nh_entry *nh, int delete_static)
{
  HSL_FN_ENTER();
		    ...
		    int nh_type)
{
  ...
  for (nh = rnh->info; nh; nh = nh->next)
    {
      if (nh->nh_type == nh_type &&
	  (ifindex == 0 || ifindex == nh->ifp->ifindex))
	{
	  break;
	}
    }
  ...
  if (nh == NULL)
        return -1;
    HSL_LOG(HSL_LOG_FIB, HSL_LEVEL_INFO, "%s %d\n", __func__, __LINE__);
    if (delete_static) {
        UNSET_FLAG(nh->flags, HSL_NH_ENTRY_STATIC);
    }
    

  ret = _hsl_fib_nh_delete2 (fib_id, nh, delete_static);
          ...
          for (nh = rnh->info; nh; nh = nh_next)
            {
              ...
              if (valid)
              ...
              else
                {
                  ...
                  if (nh->aliveCounter >= nh->ifp->u.ip.arpTimeout)
                    _hsl_fib_nh_delete2 (i, nh, 0);
                }   
            }
        }
    }
		...
		struct hsl_route_table *nh_table, u_char nh_type)
{
  ...
  for (nh = rnh->info; nh; nh = nh->next)
	...
	continue;
      ...
      if (ifindex == 0 || nh->ifp->ifindex == ifindex)
	{
	  break;
	}
    }
...
hsl_fib_nh_get (hsl_fib_id_t fib_id, hsl_ifIndex_t ifindex, hsl_prefix_t *p, u_char nh_type)
{
  ...
  nh = _hsl_fib_nh_get (ifindex, p, nh_table, nh_type);
		             ...
		             enum hsl_ipuc_nexthop_type type, hsl_ifIndex_t ifindex)
{
  ...
  if (type == HSL_IPUC_BLACKHOLE) {
		...
		hsl_fib_add_blackhole(&rnp->p, fib_id);
		HSL_FN_EXIT(0);
  }
  ...
  if (ifp->fib_id != fib_id)
    {
      HSL_LOG (HSL_LOG_FIB, HSL_LEVEL_ERROR, "FIB Id(%d) of outgoing L3 interface(%d) for NH does not match fib id(%d)\n", ifindex,
          ifp->fib_id, fib_id);
...
hsl_fib_add_connected (hsl_prefix_t *prefix, struct hsl_if *ifp)
{
  ...
  if (HSL_FIB_HWCB_CHECK(hw_add_connected_route))
    HSL_FIB_HWCB_CALL(hw_add_connected_route) (ifp->fib_id, prefix, ifp);
...
hsl_fib_delete_connected (hsl_prefix_t *prefix, struct hsl_if *ifp)
{
  ...
  if (HSL_FIB_HWCB_CHECK(hw_delete_connected_route))
    HSL_FIB_HWCB_CALL(hw_delete_connected_route) (ifp->fib_id, prefix, ifp);
...
*/ 
int
hsl_fib_add_blackhole (hsl_prefix_t *prefix, hsl_fib_id_t fib_id)
{
  /* Add this as a exception to the prefix table in hardware. */
  ...
  /* TODO check for limits in prefix table. */
  if (HSL_FIB_HWCB_CHECK(hw_add_connected_route))
    HSL_FIB_HWCB_CALL(hw_add_connected_route) (fib_id, prefix, NULL);
...
*/
int
hsl_fib_delete_blackhole (hsl_prefix_t *prefix, hsl_fib_id_t fib_id)
{
  /* Delete this as a exceptoin from the prefix table in hardware. */
  HSL_FN_ENTER();
  ...
  /* TODO check for limits in prefix table. */
  if (HSL_FIB_HWCB_CHECK(hw_delete_connected_route))
    HSL_FIB_HWCB_CALL(hw_delete_connected_route) (fib_id, prefix, NULL);
			   ...
			   hsl_ipv4Address_t *addr)
{
  ...
  ret = _hsl_fib_add_ipuc_prefix (fib_id, prefix, type, ifindex, addr ? &p : NULL, 
                           ...
                           hsl_ipv6Address_t *addr)
{
  ...
  return _hsl_fib_add_ipuc_prefix (fib_id, prefix, type, ifindex, addr ? &p : NULL,
...
#endif
	  return HSL_FIB_ERR_NH_NOT_FOUND;
	}
    }
  else
    {
				...
				hsl_ifIndex_t ifindex, u_char nh_type)
{
  ...
  if (type == HSL_IPUC_BLACKHOLE) {
		...
		hsl_fib_delete_blackhole(&rnp->p, fib_id);
		HSL_FN_EXIT(0);
  }
  ...
  if (p_hsl_fib_table->os_cb && p_hsl_fib_table->os_cb->os_prefix_delete_if)
    {
      ...
      if (ret < 0)
	{
		HSL_LOG (HSL_LOG_FIB, HSL_LEVEL_ERROR, "os_prefix_delete_if %d error %d\n", ifindex, ret);
	//  HSL_IFMGR_IF_REF_DEC (ifp);
	//  HSL_FN_EXIT(ret);
	} 
    }
			      ...
			      hsl_ifIndex_t ifindex, hsl_ipv4Address_t *addr)
{
  ...
  HSL_LOG (HSL_LOG_FIB, HSL_LEVEL_DEBUG, "delete %#0x ifindex=%d, addr = %p\n", prefix->u.prefix4, ifindex, addr);
  ...
  ret = _hsl_fib_delete_ipuc_prefix (fib_id, prefix, type, ifindex, addr ? &p : NULL, 
                              ...
                              hsl_ifIndex_t ifindex, hsl_ipv6Address_t *addr)
{
  ...
  return _hsl_fib_delete_ipuc_prefix (fib_id, prefix, type, ifindex, addr ? &p : NULL,
...
hsl_fib_handle_arp (struct hsl_if *ifp, struct hsl_eth_header *eth, struct hsl_arp *arp)
{
  ...
  if (arp->op == HSL_ARP_OP_REQUEST && !HSL_MAC_IS_ALL_ONES (eth->dmac))
      HSL_FN_EXIT(0);
      
  if ((arp->op == HSL_ARP_OP_REQUEST &&
       memcmp (eth->smac, ifp->u.ip.mac/*@jimmy ifp->u.l2_ethernet.mac*/, HSL_ETHER_ALEN) == 0) ||
      (arp->op == HSL_ARP_OP_REPLY &&
       memcmp (eth->dmac, ifp->u.ip.mac/*@jimmy ifp->u.l2_ethernet.mac*/, HSL_ETHER_ALEN) != 0)) {
	    HSL_FN_EXIT(0);
	  }
		       ...
		       unsigned char flags)
{
  ...
  HSL_LOG (HSL_LOG_FIB, HSL_LEVEL_INFO, "arp add ifindex = %d\n", ifindex);
		       ...
		       struct hal_arp_cache_entry *cache)
{
  ...
  for (rnh = hsl_route_lock_node (rn_head); rnh; rnh = hsl_route_next (rnh))
	...
	continue;
      ...
      cache[i].age_time = HSL_ARP_ALIVE_COUNTER_TO_TIMEOUT((nh->ifp->u.ip.arpTimeout - nh->aliveCounter)) ;
...
hsl_fib_handle_neigbor_discovery(struct hsl_if *ifp, char *l3_hdr_ptr, int *pkt_type)
{
  ...
  /* Learning only from advertisments. */
  ...
  ret = _hsl_fib_nh_add (ifp->fib_id, rnh, ifp, eth_src, flags, HSL_NH_TYPE_IPV6);
  if(ret < 0) 
    { 
		      ...
		      unsigned char flags)
{
  ...
  ret = hsl_fib_nh_add (ifp->fib_id, &pfx, ifp, mac_addr,
