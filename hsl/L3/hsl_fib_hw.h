...
struct hsl_fib_hw_callbacks
{
  ...
  int (*hw_add_connected_route) (hsl_fib_id_t fib_id, hsl_prefix_t *prefix, struct hsl_if *ifp);

  /* IPv4 connected route exception deletion. */
  int (*hw_delete_connected_route) (hsl_fib_id_t fib_id, hsl_prefix_t *prefix, struct hsl_if *ifp);

  /* Get maximum number of multipaths.. */
  int (*hw_get_max_multipath) (u_int32_t *ecmp);

  int (*hw_direct_prefix_add) (void *data);
  int (*hw_direct_prefix_del) (void *data);
  int (*hw_direct_nh_add) (void *data);
  int (*hw_direct_nh_del) (void *data);
};
