...
*/
#define HSL_DEFAULT_ARP_TIMER_RESOLUTION  4
...
*/
#define HSL_ARP_ALIVE_TIMEOUT_TO_COUNTER(T)       ((T)/HSL_DEFAULT_ARP_TIMER_RESOLUTION)
#define HSL_ARP_ALIVE_COUNTER_TO_TIMEOUT(C)       ((C)*HSL_DEFAULT_ARP_TIMER_RESOLUTION)
...
#define HSL_PREFIX_ENTRY_CPU		   (1 << 2)

  //void *system_info;                 /* System specific info for this prefix. */
  struct nh_system_info_s {
    int nhid;
  } system_info;
...
#define HSL_PREFIX_ENTRY_IS_CPU(rnp)  (((struct hsl_prefix_entry *)(rnp)->info)->flags & HSL_PREFIX_ENTRY_CPU)
...
#define HSL_NH_DELETE_ELIGIBLE(NH,S)                                                        \
       ((nh->refcnt == 0) && ! CHECK_FLAG (nh->flags, HSL_NH_ENTRY_VALID)                   \
...
int hsl_fib_delete_connected (hsl_prefix_t *prefix, struct hsl_if *ifp);
int hsl_fib_add_blackhole (hsl_prefix_t *prefix, hsl_fib_id_t fib_id);
int hsl_fib_delete_blackhole (hsl_prefix_t *prefix, hsl_fib_id_t fib_id);
