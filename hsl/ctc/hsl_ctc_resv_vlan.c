...
static int 
_hsl_ctc_create_vlan (hsl_vid_t vid)
{
  int ret;
...
/* Create reserved VLAN in hardware. */
static int 
_hsl_ctc_delete_vlan (hsl_vid_t vid)
{
  int ret;
...
int 
hsl_ctc_resv_vlan_db_deinit(void) 
{
  HSL_FN_ENTER (); 
...
int
hsl_ctc_resv_vlan_allocate (struct hsl_bcm_resv_vlan **retdata)
{
  struct hsl_bcm_resv_vlan *entry;
  int ret;
  ...
  /* Create VLAN in H/W. */
  ret = _hsl_ctc_create_vlan (entry->vid);
  if (ret < 0)
    {
      /* Free entry. */
      hsl_ctc_resv_vlan_deallocate (entry);
      HSL_FN_EXIT (STATUS_ERROR);
    }
...
int
hsl_ctc_resv_vlan_deallocate (struct hsl_bcm_resv_vlan *entry)
{
  HSL_FN_ENTER(); 
  ...
  _hsl_ctc_delete_vlan (entry->vid);
...
int
hsl_ctc_resv_vlan_is_allocated (hsl_vid_t vid)
{
  int ret = 0;
