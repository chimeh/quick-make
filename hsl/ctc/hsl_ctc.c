...
*/
int hsl_ctc_ifhdlr_init (void);
int hsl_linkscan_reg(void);
int hsl_ctc_ifhdlr_deinit (void);
int hsl_bcmx_init (void);
int hsl_bcmx_deinit (void);
extern int  hsl_ctc_port_attach_cb (int lport, int unit, int port, uint32_t flags);
...
*/
int hsl_bcm_filter_type_get(void)
{
  HSL_FN_ENTER(); 
...
static int _hsl_bcm_filter_init(void)
{
  int ret; 
...
_hsl_attach_ports (int unit)
{
  ...
  BCMX_FOREACH_LPORT(lport) {
      ...
      if (BCMX_LPORT_BCM_UNIT(lport) == unit)
      {
         ...
         port = (int ) BCMX_LPORT_BCM_PORT(lport);
         val = &value;
		 /*
         if ((val = (int *)hsl_bcm_port_attach_cb (lport, unit, port, flags)) 
               != NULL)
         {
...
hsl_ctc_init (void)
{
  ...
  /* Initialize Packet driver, ctc Tx/Rx. */
  SYSTEM_INIT_CHECK(hsl_ctc_pkt_init (), "ctc pkt Rx init");
  if (rv < 0)
    {
...
//  str = (char *) sal_config_get ("station_mac_base");
  ...
  SYSTEM_INIT_CHECK(hsl_bcm_ifmap_init (max_fe, max_ge, max_xe, (u_char *)base_mac), "ctc ifmap");

  /* Initialize interface handler. */
  SYSTEM_INIT_CHECK(hsl_ctc_ifhdlr_init (), "ctc ifhdlr init");

  /* Reserved VLAN database init. */
  SYSTEM_INIT_CHECK(hsl_ctc_resv_vlan_db_init (HSL_MVL_RESV_VLAN_FIRST_DEFAULT, HSL_MVL_RESV_VLAN_NUM_DEFAULT), "ctc reserved vlan db init");
...
hsl_hw_deinit (void)
{
  ...
  hsl_ctc_ifhdlr_deinit ();

  /* Deinitialize reserved VLAN database. */
  hsl_ctc_resv_vlan_db_deinit ();
...
*/
int
hsl_hw_mac_base_set (mac_addr_t mac)
{
