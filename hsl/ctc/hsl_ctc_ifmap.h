...
#define HSL_CTC_TRUNK_2_LPORT(trunk)         (0x80000000 | (trunk))
#define HSL_CTC_LPORT_2_TRUNK_ID(lport)      (lport & 0x7fffffff)

#define HSL_CTC_LPORT_SET(lport, vid, ctc_gport) \
do {                                             \
	lport = (((vid) << 16) | (ctc_gport));   \
} while(0)
...
/* Function prototypes. */
int hsl_bcm_ifmap_lport_register (int lport, unsigned int flags, char *name, u_char *mac);
int hsl_bcm_ifmap_lport_unregister (int lport);
int hsl_bcm_ifmap_init (int max_fe, int max_ge, int max_xe, u_char *mac_base);
int hsl_bcm_ifmap_deinit (void);
int hsl_bcm_ifmap_ifindex_set (int lport, hsl_ifIndex_t ifindex);
int hsl_ctc_ifmap_if_map (int gport, struct hsl_if *ifp);
struct hsl_if *hsl_bcm_ifmap_if_get (int lport);
int hsl_ctc_ifmap_if_set (int lport, struct hsl_if *ifp);
u_int32_t hsl_bcm_port_u2l (int modid, int unit, int port);
void hsl_bcm_port_l2u (u_int32_t uport, int *modid, int *unit, int *port);
int hsl_bcm_ifmap_init (int max_fe, int max_ge, int max_xe, uint8_t mac_base[6]);
int hsl_bcm_ifmap_deinit (void);
char *hsl_bcm_ifmap_svi_mac_get (void);

int
hsl_bcm_ifmap_if_map (int gport, struct hsl_if *ifp);
