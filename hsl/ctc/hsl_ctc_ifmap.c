...
*/
int hsl_bcm_ifmap_lport_register (int gport, unsigned int flags, char *name, u_char *mac)
{
    ...
    int unit = -1;
    char cname[HSL_IFNAM_SIZE + 1] = { 0 };
    ln.lport = gport;
    
    if(p_hsl_bcm_ifmap->lport_map_tree == NULL) {
        printk("[%s]: lport_map_tree is NULL\r\n", __func__);
        return -1;
    }

//    printk("[%s]: gport: %d, flags: %d, p_hsl_bcm_ifmap->lport_map_tree: %p \r\n", __func__, gport, flags, p_hsl_bcm_ifmap->lport_map_tree );

    node = hsl_avl_lookup (p_hsl_bcm_ifmap->lport_map_tree, &ln);
    if (! node) {
        /* No entry found for this lport. Assign a name and index. */
        if (flags & CTC_PORT_F_FE) {
        ...
        if(ctc_get_portname_by_gport(type, gport, cname, sizeof(cname)) == NULL) {
            /* shouldn't be happened */
            hsl_avl_delete(p_hsl_bcm_ifmap->lport_map_tree, lm);
        
//            printk("[%s]: get port<%d> name failed \r\n", __func__, gport);
            return -1;
        }
//        printk ( "[%s]: gport: %2d, name: %s \r\n", __func__, gport, cname);

        memcpy(name, cname, sizeof(cname));
        /* Get the mac address. */
    ...
    HSL_LOG (HSL_LOG_PLATFORM, HSL_LEVEL_INFO, "Gport(%d) registered\n", gport);
...
*/
int
hsl_bcm_ifmap_lport_unregister (int gport)
{
  ...
  /* Free the port index. */
  if (lm->type == CTC_PORT_F_FE)
    arr = p_hsl_bcm_ifmap->fe_index_table;
  else if (lm->type == CTC_PORT_F_GE)
    arr = p_hsl_bcm_ifmap->ge_index_table;
  else if (lm->type == CTC_PORT_F_XE)
    arr = p_hsl_bcm_ifmap->xe_index_table;
...
*/
int
hsl_bcm_ifmap_mapped_name_get (int gport, int unit, int port, uint32_t flags, char *name)
{
  return -1;
}
...
*/
int
hsl_ctc_ifmap_if_set (int gport, struct hsl_if *ifp)
{
  struct hsl_bcm_ifmap_lport ln, *lm;
  struct hsl_avl_node *node;
...
int
hsl_ctc_ifmap_if_map (int gport, struct hsl_if *ifp)
{
    struct hsl_bcm_ifmap_lport *lm;
...
*/
struct hsl_if *
hsl_bcm_ifmap_if_get (int gport)
{
  struct hsl_bcm_ifmap_lport ln, *lm;
  struct hsl_avl_node *node;
...
*/
int 
hsl_bcm_ifmap_init (int max_fe, int max_ge, int max_xe, uint8_t mac_base[6])
{
  int ret = 0, i;
  //u_char *prev;
  ...
  memcpy (mac_addr, dev->dev_addr, HSL_ETHER_ALEN);
  ...
  for (i = 0; i < p_hsl_bcm_ifmap->max_macindexes; i++)
      ...
      if (! p_hsl_bcm_ifmap->mac_array[i])
	{
	  ...
	  memcpy ((u_char *)p_hsl_bcm_ifmap->mac_array[i], mac_addr, HSL_ETHER_ALEN);
...
{
  int i;
  struct hsl_avl_node *tmp_node, *node;
  struct hsl_bcm_ifmap_lport *lport;

  if (! p_hsl_bcm_ifmap)
    return 0;

  node = hsl_avl_top (p_hsl_bcm_ifmap->lport_map_tree);
  while (node)
    {
      if (node->left)
	{
      ...
      if (node != NULL)
	{
      ...
      lport = HSL_AVL_NODE_INFO(tmp_node);
      if (! lport)
	continue;
      
      /* Free lport name mapping. */
      _free_ifmap_lport (&lport);
    }
