...
hsl_os_if_init (void)
{
  ...
  printk("add eth0 mtu=%d, hw addr=%08x\n", dev->mtu, *(unsigned int *)dev->dev_addr);
  hsl_ifmgr_L3_cpu_if_register ("eth0", dev->ifindex, dev->mtu, 0, dev->flags, dev->dev_addr, dev);
  

  hsl_ifmgr_set_flags(NULL, dev->ifindex, dev->flags|IFF_UP|IFF_RUNNING);
...
hsl_os_if_deinit (void)
{
  ...
  dev = dev_get_by_name (&init_net, loopback_if);
  if (!dev)
    return 0;
                        ...
                        int hwaddrlen, hsl_ifIndex_t *ifindex)
{
  ...
  /* Create netdevice. */
  dev = hsl_eth_drv_create_netdevice (ifp, hwaddr, hwaddrlen, *ifindex);
...
hsl_os_l3_if_mtu_set (struct hsl_if *ifp, int mtu)
{
  ...
  oldfs = get_fs ();
  set_fs (get_ds ());
  ret = devinet_ioctl (&init_net, SIOCSIFMTU, &req);
  set_fs (oldfs);
...
hsl_os_l3_if_hwaddr_set (struct hsl_if *ifp, int hwaddrlen, u_char *hwaddr)
{
  ...
  if (! dev || ! dev->netdev_ops->ndo_set_mac_address)
    return -1;
  ...
  oldfs = get_fs ();
  set_fs (get_ds ());
  ret = devinet_ioctl (&init_net, SIOCSIFHWADDR, &req);
  set_fs (oldfs);
...
hsl_os_l3_if_flags_set (struct hsl_if *ifp, unsigned long flags)
{
  ...
  oldfs = get_fs ();
  set_fs (get_ds ());
  ret = devinet_ioctl (&init_net, SIOCSIFFLAGS, &req);
  set_fs (oldfs);
...
_hsl_os_l3_ipv4_nh_add (struct hsl_nh_entry *nh)
{
  ...
  if (CHECK_FLAG (nh->flags, HSL_NH_ENTRY_STATIC))
	...
	return -1;
	  ...
	  memcpy(req.arp_ha.sa_data, nh->mac, HSL_ETHER_ALEN);
      strncpy (req.arp_dev, dev->name, sizeof (req.arp_dev));
      
      /* Delete arp entry. */
	  net = dev_net(dev);
      arp_req_set (net, &req, dev);
    }
		      ...
		      unsigned int flags, int cmd)
{
  ...
  if (! rnp || ! ifp){
  	printk("_hsl_os_l3_prefix_if error\n");
    return -1;
  }
...
*/
int 
hsl_os_ipv4_mc_pim_deinit (void)
{
  rtnl_lock ();
 // mroute_do_assert = 0;
 // mroute_do_pim = 0;
...
hsl_fib_os_cb_register (void)
{
  ...
  OS_FIBCB(os_prefix_add)       = NULL;// hsl_os_l3_prefix_add;
  OS_FIBCB(os_prefix_add_if)    = NULL;// hsl_os_l3_prefix_add_if;
  OS_FIBCB(os_prefix_delete)    = NULL;// hsl_os_l3_prefix_delete;
  OS_FIBCB(os_prefix_delete_if) = NULL;// hsl_os_l3_prefix_delete_if;
  OS_FIBCB(os_nh_add)           = NULL;// hsl_os_l3_nh_add;
  OS_FIBCB(os_nh_delete)        = NULL;// hsl_os_l3_nh_delete;
...
{
  OS_IFCB(os_if_init)                     = NULL;//hsl_os_if_init;
  OS_IFCB(os_if_deinit)                   = NULL;//hsl_os_if_deinit;
  OS_IFCB(os_l2_if_flags_set)             = NULL; /* No L2 ports in OS. */
  OS_IFCB(os_l2_if_flags_unset)           = NULL; /* No L2 ports in OS. */
  OS_IFCB(os_l3_if_configure)             = NULL;//hsl_os_l3_if_configure;
  OS_IFCB(os_l3_if_unconfigure)           = NULL;//hsl_os_l3_if_unconfigure;
  OS_IFCB(os_l3_if_mtu_set)               = NULL;//hsl_os_l3_if_mtu_set;
  OS_IFCB(os_l3_if_hwaddr_set)            = NULL;//hsl_os_l3_if_hwaddr_set;
  OS_IFCB(os_if_secondary_hwaddrs_delete) = NULL;//hsl_os_if_secondary_hwaddrs_delete;
  OS_IFCB(os_if_secondary_hwaddrs_add)    = NULL;//hsl_os_if_secondary_hwaddrs_add;
  OS_IFCB(os_if_secondary_hwaddrs_set)    = NULL;//hsl_os_if_secondary_hwaddrs_set;
  OS_IFCB(os_l3_if_flags_set)             = NULL;//hsl_os_l3_if_flags_set;
  OS_IFCB(os_l3_if_flags_unset)           = NULL;//hsl_os_l3_if_flags_unset;
  OS_IFCB(os_l3_if_address_add)           = NULL;//hsl_os_l3_if_address_add;
  OS_IFCB(os_l3_if_address_delete)        = NULL;//hsl_os_l3_if_address_delete;
...
int hsl_ipv4_mc_os_cb_register(void)
{
  ...
  OS_MCCB(os_ipv4_mc_pim_init)   = NULL;//hsl_os_ipv4_mc_pim_init;
  OS_MCCB(os_ipv4_mc_pim_deinit) = NULL;//hsl_os_ipv4_mc_pim_deinit;
  OS_MCCB(os_ipv4_mc_route_add)  = NULL;//hsl_os_ipv4_mc_route_add;
  OS_MCCB(os_ipv4_mc_route_del)  = NULL;//hsl_os_ipv4_mc_route_del;
  OS_MCCB(os_ipv4_mc_vif_add)    = NULL;//hsl_os_ipv4_mc_vif_add;
  OS_MCCB(os_ipv4_mc_vif_del)    = NULL;//hsl_os_ipv4_mc_vif_del;
