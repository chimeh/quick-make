...
#ifndef _HSL_CTC_PORT_VLAN_H_
#define _HSL_CTC_PORT_VLAN_H_
...
struct hsl_bcm_resv_vlan_db
{
  ...
  ipi_sem_id                  mutex;           /* Protection mutex.  */
...
int hsl_ctc_resv_vlan_db_deinit(void);

/* Allocate a reserved vlan. 
   Returns a void * which should be used for freeing the entry.
   Also returns the VID. */
int hsl_ctc_resv_vlan_allocate (struct hsl_bcm_resv_vlan **retdata);

/* Free a allocated reserved vlan. */
int hsl_ctc_resv_vlan_deallocate (struct hsl_bcm_resv_vlan *resv_vlan);

/* Check if a VLAN is reserved. 
   Returns 1 if the VID is reserved.
   Returns 0 if the VID is unreserved. */
int hsl_ctc_resv_vlan_is_allocated (hsl_vid_t vid);

#endif /* _HSL_CTC_PORT_VLAN_H_ */
