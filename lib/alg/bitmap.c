                  ...
                  u_int32_t max_index)
{
  ...
  block->map = bitmap_block_create_map (bitmap, TRUE);
  if (! block->map)
    {
...
//只有block指针，map为NULL且没有设置已使用标志位。(释放某个index,map全部空闲，被释放为NULL的情况)
/* Will return NULL if no nodes were found that were "marked for release." */
struct bitmap_block *
  ...
  for (i = 0; i < BITMAP_HASH_SIZE; i++)
    for (block = bitmap->hash[i]; block; block = block->next)
      if (! block->map && ! CHECK_FLAG (block->flags, BITMAP_BLOCK_FLAG_USED))
        if ((block->map = bitmap_block_create_map (bitmap, TRUE)) != NULL)
          {
                        ...
                        u_int32_t curr_idx, u_int32_t *next_idx)
{
  ...
  for (i = bit + 1; i < INTEGER_BITS; i++)
    {
      if (CHECK_FLAG (block->map[offset], (1 << i)))
        {
          *next_idx = ((bitmap->block_size * block->id)
                             + (offset * INTEGER_BITS) + i);
          return BITMAP_SUCCESS;
        }
    }
  ...
  /* Go through the array of integers, and find the first set bit, which
     will indicate that this bit's corresponding index is empty.  */
  for (i = offset + 1; i < bitmap->array_size; i++)
    {
      /* Find fist bit call.  */
      first_bit_set = zffs (block->map[i]);
...
bitmap_passive_new (int block_size, u_int32_t min_index, u_int32_t max_index)
{
  ...
  /* 0 在bitmap中当作特殊值使用，block初始化和map用完后，first_free_index都为0 */
  if (min_index == 0)
    return NULL;
...
/* Helper routine to create a bitmap with block size 1280, min_index 1
   and max_index UINT_MAX. */
struct bitmap *
bitmap_default_new ()
{
  return bitmap_new (1280, 1, UINT_MAX);
}
...
bitmap_add_index (struct bitmap *bitmap, u_int32_t index)
{
  ...
  if (block->first_free_index == 0)
    offset = 0;
  else if (block->first_free_index < index)
    return BITMAP_SUCCESS;
  else 
    offset = (block->first_free_index % bitmap->block_size) / INTEGER_BITS;
  ...
  if ((next_block != NULL))
    {
      if ( !next_block->map && 
        ...
        {
          if ((next_block->map = bitmap_block_create_map (bitmap, TRUE))
                                       != NULL )
            {
  ...
  if ((prev_block != NULL ))
    {
      if (!prev_block->map
        ...
        {
          if ((prev_block->map = bitmap_block_create_map (bitmap , TRUE))
                                                                        != NULL)
            {
