...
zclient_flush_data(struct thread *thread)
{
  ...
  switch (buffer_flush_available(zclient->wb, zclient->sock))
    ...
    case BUFFER_ERROR:
      ...
      break;
    case BUFFER_PENDING:
      zclient->t_write = thread_add_write(zclient->t_master, zclient_flush_data,
...
zclient_send_message(struct zclient *zclient)
{
  ...
  switch (buffer_write(zclient->wb, zclient->sock, STREAM_DATA(zclient->obuf),
    ...
    case BUFFER_EMPTY:
      ...
      break;
    case BUFFER_PENDING:
      THREAD_WRITE_ON(zclient->t_master, zclient->t_write,
...
zclient_event (enum event event, struct zclient *zclient)
{
  switch (event)
    ...
    case ZCLIENT_SCHEDULE:
      if (! zclient->t_connect)
	zclient->t_connect =
	  thread_add_event (zclient->t_master, zclient_connect, zclient, 0);
      break;
	...
	zlog_debug ("zclient connect schedule interval is %d", 
      ...
      if (! zclient->t_connect)
	zclient->t_connect = 
	  thread_add_timer (zclient->t_master, zclient_connect, zclient,
			    zclient->fail < 3 ? 10 : 60);
      break;
    case ZCLIENT_READ:
      zclient->t_read = 
	thread_add_read (zclient->t_master, zclient_read, zclient, zclient->sock);
      break;
    }
}
