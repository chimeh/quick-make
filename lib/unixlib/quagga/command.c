/*
   $Id: command.c,v 1.1 2010-02-01 08:59:43 zhengjiangyong Exp $
...
//by zzy
static enum match_type
cmd_slot_port_match (const char *str)
{
  const char *sp;
  //int dots = 0;
  char buf[4];
	...
	if (index >= vector_active (cmd_element->strvec))
	...
	else
	  {
	      ...
	      if ((desc = vector_slot (descvec, j)))
		{
		  ...
		  else if (CMD_SLOT_PORT (str))
		    {
		      if (cmd_slot_port_match (command))
			{
	...
	else
	  {
	      ...
	      if ((desc = vector_slot (descvec, j)))
		{
		  ...
		  if (CMD_VARARG (str))
		    ...
		    {
		      if (cmd_slot_port_match (command) == exact_match)
			{
			  if (match_type < slot_port_match)
			    match_type = slot_port_match;
			  matched++;
	...
	for (j = 0; j < vector_active (descvec); j++)
	  if ((desc = vector_slot (descvec, j)))
		...
		case ipv4_prefix_match:
		  ...
		  break;
		case slot_port_match:
		  if ((ret = cmd_slot_port_match (command)) != no_match)
		    {
		      if (ret == partly_match)
			return 2;	/* There is incomplete match. */
...
cmd_entry_function_desc (const char *src, const char *dst)
{
  ...
  if (CMD_SLOT_PORT (dst))
    {
      if (cmd_slot_port_match (src))
	return dst;
      else
	return NULL;
    }
...
/* Configration make from file. */
int
config_from_string (struct vty *vty, char *one_cmd_line)
{
  int ret;
  vector vline;

  //while (fgets (vty->buf, VTY_BUFSIZ, fp))
  strcpy(vty->buf,one_cmd_line);
    {
      vline = cmd_make_strvec (vty->buf);
...
cmd_init (int terminal)
{
  ...
  install_element (ENABLE_NODE, &config_quit_cmd);
