...
/* exec one cmd */
void vty_read_str(char *one_cmd_str, int node)
{
	int ret;
	struct vty *vty;
	...
	ret = config_from_string(vty, one_cmd_str);
...
//            int size = 1024;
            ...
            va_end(args);
			vty_out(v, "%s", buf);
			if ((v->fd>0)&&(v->obuf!=NULL) && v->type == VTY_TERM)
			{
				vty_event (VTY_WRITE, v->fd, v);
			}
